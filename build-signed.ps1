# build-signed.ps1
# Script to build and sign the Electron app with a Sectigo USB Token
...
if (-not $env:WIN_CERTIFICATE_SHA1) {
    ...
    if (-not $certThumbprint) {
        ...
        # Pause is handled by build.bat, so we just exit
        exit 1
    }
...
try {
    # Run the npm build command WITHOUT the invalid flag
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        # Verify if the output file exists (Adjust version/name as needed for your specific project)
        $exeFiles = Get-ChildItem "dist" -Filter "*.exe" | Where-Object { $_.Name -notlike "*uninstaller*" }
        if ($exeFiles) {
